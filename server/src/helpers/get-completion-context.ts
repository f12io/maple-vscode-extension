 ...
 */
export function getCompletionContext(word: string): CompletionContext {
  // Empty word - show initial suggestions
  if (!word) {
    return { types: ['initial'], parsed: parseClass('') };
  }
  ...
  try {
    ...
    const parsed = parseClass(word);
    if (word.startsWith('@')) {
      return { types: ['media-query'], parsed };
    }
    if (word.startsWith('&')) {
      return { types: ['pseudo-class'], parsed };
    }
    return { types: ['property'], parsed };
  }
}
